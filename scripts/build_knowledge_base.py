...
import os, re, json, glob, argparse, subprocess, sys
...
def main():
    ...
    # ── Stage 1.5: KB Feedback Context 자동 생성 ──────────────────────
    # 발동 조건: 새 레코드 추가됨 OR 배치 모드(--date 없음)
    if added_d > 0 or args.date is None:
        script = os.path.join(APA_ROOT, 'scripts', 'kb_feedback_context.py')
        if os.path.exists(script):
            print("→ Stage 1.5: KB Feedback Context 생성 중...")
            result = subprocess.run(
                [sys.executable, script, '--days', '14'],
                capture_output=True, text=True
            )
            if result.returncode == 0:
                print(result.stdout.strip())
            else:
                print(f"⚠️  Stage 1.5 오류:\n{result.stderr.strip()}")
        else:
            print(f"⚠️  Stage 1.5 스크립트 없음: {script}")
