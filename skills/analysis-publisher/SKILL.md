...
## Phase 4.5: APV 품질 게이트 (절대 규칙)

Phase 4 산출물(HTML 2개) 생성 직후, Phase 5(인덱스 갱신) 진입 전에 반드시 `apv-validator` 스킬을 실행한다.

**대상 파일 (2개):**
- `web/analysis/<slug>-en.html`
- `web/analysis/<slug>-ko.html`

**통과 기준:** 12개 체크포인트 중 10개 이상 PASS.
**FAIL 시:** 해당 파일 수정 → APV 재실행 → PASS 확인 후 Phase 5 진행.
**편집 서명 필수:** APV Point 12(편집 서명) FAIL 시 `</article>` 직전에 `<blockquote class="editorial-note">` 블록 삽입 후 재검사.
